...
EasingDemo = function(p) { if( p === $_ ) return; {
	easel.Scene.apply(this,[]);
}}
EasingDemo.__name__ = ["EasingDemo"];
EasingDemo.__super__ = easel.Scene;
for(var k in easel.Scene.prototype ) EasingDemo.prototype[k] = easel.Scene.prototype[k];
EasingDemo.main = function() {
	var director = new easel.Director(js.Lib.document.getElementById("screen"));
	EasingDemo.assets.onComplete._slots.push(function(_) {
		director.init(new EasingDemo());
	});
	EasingDemo.assets.load(["man.png"]);
}
EasingDemo.prototype.load = function() {
	this.add(new easel.display.FilledSprite("#000000"));
	var man = new easel.display.ImageSprite(EasingDemo.assets.get("man.png"));
	man.setCenterX(man.boundingBox[2] / 2);
	man.setCenterY(man.boundingBox[3] / 2);
	{
		man.x = 50;
		man._isTransformDirty = true;
		50;
	}
	{
		man.y = 50;
	...
	options.set("Linear",easel.tasks.Tween.LINEAR);
	options.set("Quad EaseIn",easel.tasks.Tween.QUAD);
	var select = js.Lib.document.getElementById("easings");
	...
	var jumpy = js.Lib.document.getElementById("jumpy");
	this.onClick._slots.push(function(event) {
		var x = event.clientX;
		var y = event.clientY;
		var easing = options.get(select.value);
		man.removeAllTasks();
		man.addTask(new easel.tasks.MoveTo(man,x,y,1,easing));
		if(jumpy.checked) {
			man.addTask(new easel.tasks.RotateBy(man,2 * Math.PI,1,easing));
			man.addTask(new easel.tasks.Sequence([new easel.tasks.ScaleTo(man,2,2,0.5,easing),new easel.tasks.ScaleTo(man,1,1,0.5,easing)]));
		}
	});
}
EasingDemo.prototype.__class__ = EasingDemo;
Std = function() { }
Std.__name__ = ["Std"];
Std["is"] = function(v,t) {
...
easel.tasks.RotateBy = function(sprite,by,seconds,easing) { if( sprite === $_ ) return; {
	easel.tasks.Tween.apply(this,[seconds,easing]);
	this._sprite = sprite;
	this._by = by;
}}
easel.tasks.RotateBy.__name__ = ["easel","tasks","RotateBy"];
easel.tasks.RotateBy.__super__ = easel.tasks.Tween;
for(var k in easel.tasks.Tween.prototype ) easel.tasks.RotateBy.prototype[k] = easel.tasks.Tween.prototype[k];
easel.tasks.RotateBy.linear = function(sprite,by,seconds) {
	return new easel.tasks.RotateBy(sprite,by,seconds,easel.tasks.Tween.LINEAR);
}
easel.tasks.RotateBy.prototype._by = null;
easel.tasks.RotateBy.prototype._from = null;
easel.tasks.RotateBy.prototype._sprite = null;
easel.tasks.RotateBy.prototype.begin = function() {
	this._from = this._sprite.rotation;
}
easel.tasks.RotateBy.prototype.tick = function() {
	this._sprite.setRotation(this._easing(this._elapsed,this._from,this._from + this._by,this._duration));
}
easel.tasks.RotateBy.prototype.__class__ = easel.tasks.RotateBy;
...
EasingDemo.assets = new easel.util.AssetPack("./");
js.Lib.onerror = null;
$Main.init = EasingDemo.main();
