...
TextDemo = function(p) { if( p === $_ ) return; {
	easel.Scene.apply(this,[]);
}}
TextDemo.__name__ = ["TextDemo"];
TextDemo.__super__ = easel.Scene;
for(var k in easel.Scene.prototype ) TextDemo.prototype[k] = easel.Scene.prototype[k];
TextDemo.main = function() {
	var director = new easel.Director(js.Lib.document.getElementById("screen"));
	director.init(new TextDemo());
}
TextDemo.prototype.load = function() {
	this.add(new easel.display.FilledSprite("#f0f0f0"));
	...
	label.setY(this.director.ctx.canvas.width / 2 + 80);
	{
		label.shadowBlur = 2;
		label._isContentsDirty = true;
		2;
	...
	var center = new easel.display.CircleSprite(2);
	{
		...
		center._isContentsDirty = true;
		"#0000ff";
	}
	center.setX(label.x);
	center.setY(label.y);
	this.add(center);
	this.addTask(new easel.tasks.Repeat(new easel.tasks.RotateBy(label,2 * Math.PI,20,easel.tasks.Tween.LINEAR)));
	var cacheAsBitmapCtrl = js.Lib.document.getElementById("cacheAsBitmap");
	(cacheAsBitmapCtrl.onchange = function(event) {
		label.setCacheAsBitmap(cacheAsBitmapCtrl.checked);
	})(null);
	var enableShadowCtrl = js.Lib.document.getElementById("enableShadow");
	(enableShadowCtrl.onchange = function(event) {
		label.setShadowColor((enableShadowCtrl.checked?"#909090":null));
	})(null);
	var fontCtrl = js.Lib.document.getElementById("font");
	(fontCtrl.onchange = function(event) {
		label.setFont(fontCtrl.value);
	})(null);
	var alignCtrl = js.Lib.document.getElementById("align");
	(alignCtrl.onchange = function(event) {
		label.setAlign(alignCtrl.value);
	})(null);
	var baselineCtrl = js.Lib.document.getElementById("baseline");
	(baselineCtrl.onchange = function(event) {
		label.setBaseline(baselineCtrl.value);
	})(null);
}
TextDemo.prototype.__class__ = TextDemo;
